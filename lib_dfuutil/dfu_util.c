...
static void probe_configuration(dfu_util_t* util, libusb_device *dev, struct libusb_device_descriptor *desc)
{
	...
	for (cfg_idx = 0; cfg_idx != desc->bNumConfigurations; cfg_idx++) {
		...
		if (ret != 0)
			return;
		if (util->match_config_index > -1 && util->match_config_index != cfg->bConfigurationValue) {
...
found_dfu:
		...
		for (intf_idx = 0; intf_idx < cfg->bNumInterfaces;
		     intf_idx++) {
			if (util->match_iface_index > -1 && util->match_iface_index != intf_idx)
				continue;
			...
			for (alt_idx = 0;
				...
				if (dfu_mode &&
				    util->match_iface_alt_index > -1 && util->match_iface_alt_index != alt_idx)
					continue;

				if (dfu_mode) {
					if ((util->match_vendor_dfu >= 0 && util->match_vendor_dfu != desc->idVendor) ||
					    (util->match_product_dfu >= 0 && util->match_product_dfu != desc->idProduct)) {
						continue;
					}
				} else {
					if ((util->match_vendor >= 0 && util->match_vendor != desc->idVendor) ||
					    (util->match_product >= 0 && util->match_product != desc->idProduct)) {
						continue;
					}
				}
				...
				if (dfu_mode &&
					util->match_iface_alt_name != NULL && strcmp(alt_name, util->match_iface_alt_name))
					continue;

				if (dfu_mode) {
					if (util->match_serial_dfu != NULL && strcmp(util->match_serial_dfu, serial_name))
						continue;
				} else {
					if (util->match_serial != NULL && strcmp(util->match_serial, serial_name))
						continue;
				}
...
void probe_devices(dfu_util_t* util)
{
	libusb_device **list;
	ssize_t num_devs;
	ssize_t i;

	num_devs = libusb_get_device_list(util->ctx, &list);
	for (i = 0; i < num_devs; ++i) {
		struct libusb_device_descriptor desc;
		struct libusb_device *dev = list[i];

		if (util->match_path != NULL && strcmp(get_path(dev), util->match_path) != 0)
			continue;
		if (libusb_get_device_descriptor(dev, &desc))
			continue;
		probe_configuration(util, dev, &desc);
	}
	libusb_free_device_list(list, 0);
}

void disconnect_devices(dfu_util_t* util)
{
	struct dfu_if *pdfu;
	struct dfu_if *prev = NULL;
	struct dfu_if* dfu_root_dev = util->dfu_root;

	for (pdfu = dfu_root_dev; pdfu != NULL; pdfu = pdfu->next) {
		free(prev);
...
void list_dfu_interfaces(dfu_util_t* util)
{
	struct dfu_if *pdfu;

	for (pdfu = util->dfu_root; pdfu != NULL; pdfu = pdfu->next)
		print_dfu_if(pdfu);
}
...
void dfu_util_init(dfu_util_t* util)
{
	...
	util->match_path = NULL;
	util->match_vendor = -1;
	util->match_product = -1;
	util->match_vendor_dfu = -1;
	util->match_product_dfu = -1;
	util->match_config_index = -1;
	util->match_iface_index = -1;
	util->match_iface_alt_index = -1;
	util->match_iface_alt_name = NULL;
	util->match_serial = NULL;
	util->match_serial_dfu = NULL;
}
